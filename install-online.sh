...
repository=${GNOME_ASCII_SAVER_REPOSITORY:-RobbyBobby77/gnome-ascii-saver}
requested_version=${GNOME_ASCII_SAVER_VERSION:-latest}
release_base_url=${GNOME_ASCII_SAVER_RELEASE_BASE_URL:-"https://github.com/$repository/releases"}
installer_args=()
temporary_dir=
...
Download a GNOME ASCII Saver GitHub release, verify its SHA-256 checksum, and
run its user-local installer. With no options, the latest stable release is used.
...
Environment overrides:
  GNOME_ASCII_SAVER_VERSION           Release version, or "latest"
  GNOME_ASCII_SAVER_REPOSITORY        GitHub owner/repository
  GNOME_ASCII_SAVER_RELEASE_BASE_URL  Release URL base (intended for testing)
  GNOME_ASCII_SAVER_NO_SESSION=1      Prevent installer session actions
EOF
}
...
while (($#)); do
    case "$1" in
        --version)
            ...
            shift
            ;;
        --no-start|--non-interactive) installer_args+=("$1") ;;
        --)
            shift
            installer_args+=("$@")
            break
...
if [[ "$requested_version" == latest ]]; then
    archive_name=gnome-ascii-saver.tar.gz
    ...
    archive_name="gnome-ascii-saver-$version.tar.gz"
    download_base="$release_base_url/download/v$version"
fi
checksum_name="$archive_name.sha256"

temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/gnome-ascii-saver-download.XXXXXX")
...
printf 'Downloading GNOME ASCII Saver %s...\n' "$requested_version"
